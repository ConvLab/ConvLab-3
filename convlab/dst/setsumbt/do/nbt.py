...
def main(args=None, config=None):
    ...
    if 'pytorch_model.bin' in paths and 'config.json' in paths:
        ...
        config = ConfigClass.from_pretrained(args.model_name_or_path, local_files_only=True)
    else:
    ...
    model = SetSumbtModel.from_pretrained(args.model_name_or_path, config=config, local_files_only=True)
    model = model.to(device)

    # Create Tokenizer and embedding model for Data Loaders and ontology
    encoder = CandidateEncoderModel.from_pretrained(config.candidate_embedding_model_name, local_files_only=True)
    tokenizer = Tokenizer.from_pretrained(config.tokenizer_name, config=config, local_files_only=True)
