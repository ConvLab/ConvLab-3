...
class SCBART(NLG):
    ...
    def generate(self, action, conduct='neutral'):
        ...
        prompt = PROMPT_TEMPLATE.replace(ACT_PLACEHOLDER, action_str).replace(CON_PLACEHOLDER, conduct)
    ...
    def _inference(self, act_str):
        ...
        with torch.no_grad():
            ...
            output_ids = self.model.generate(
                ...
                do_sample=True, 
                temperature=0.9)
            nlg_outputs = self.tokenizer.batch_decode(output_ids.cpu(), skip_special_tokens=True)
        return nlg_outputs
